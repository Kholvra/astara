...
export const Module1Explore: React.FC<Module1Props> = ({
  ...
  return (
    <div className="relative h-full w-full overflow-hidden bg-slate-100 font-sans select-none">
      {/* 1. Map Canvas Body */}
      <div className="absolute inset-0 z-0 h-full w-full">{children}</div>

      {/* 2. Top Floating Search Bar Trigger */}
      <header className="absolute top-11 right-0 left-0 z-20 px-4">
        <button
      ...
      >
        ...
        <div className="no-scrollbar flex [scrollbar-width:none] items-center gap-2.5 overflow-x-auto pb-3 [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden">
        ...
        {selectedStop && (
          <button
            ...
            className="mb-1 flex w-full cursor-pointer items-center justify-between rounded-2xl border border-slate-200/90 bg-white p-3.5 text-left transition-all hover:border-emerald-300 hover:shadow-xs active:scale-[0.99]"
          >
            <div className="flex min-w-0 items-center gap-3">
              ...
              <div className="min-w-0">
                <div className="truncate text-[14px] leading-snug font-bold text-slate-900">
                  {selectedStop.title}
                </div>
                <div className="truncate text-[11.5px] font-medium text-slate-500">
                  {selectedStop.walkTimeMinutes &&
                  selectedStop.walkDistanceMeters
