...
import { ArrowRight, Bus, ChevronUp, Footprints } from "lucide-react";
...
export const RouteSummarySheet = ({
  ...
  return (
    <div className="flex min-w-0 flex-col gap-3">
      ...
      {/* Steps Section: Either CTA button to expand bottom card (when closed), or direct list of steps (when opened) */}
      {!isStepsOpen ? (
        <button
          type="button"
          id="route-steps-toggle"
          onClick={() => onStepsToggle?.(true)}
          className="flex min-h-11 w-full cursor-pointer items-center justify-center gap-2 rounded-2xl border border-emerald-200/80 bg-emerald-50/70 px-4 py-2.5 text-xs font-bold text-emerald-800 transition-all hover:bg-emerald-100/80 active:scale-[0.99] focus-visible:ring-2 focus-visible:ring-emerald-500 focus-visible:outline-none"
        >
          <Footprints className="h-4 w-4 text-emerald-700" />
          <span>Buka {model.steps.length} Langkah Perjalanan</span>
          <ChevronUp className="h-4 w-4 text-emerald-700" />
        </button>
      ) : (
        <div
          id="route-steps-content"
          className="min-w-0 border-t border-slate-100 pt-3"
        >
          <div className="mb-3 flex items-center justify-between">
            ...
            <button
              type="button"
              onClick={() => onStepsToggle?.(false)}
              className="cursor-pointer text-xs font-semibold text-slate-400 transition-colors hover:text-slate-600 focus-visible:ring-2 focus-visible:ring-emerald-500 focus-visible:outline-none"
            >
              Tutup detail
            </button>
          </div>
