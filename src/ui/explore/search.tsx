...
export const Module2Search: React.FC<Module2Props> = ({
  ...
  const [ambiguousItem, setAmbiguousItem] = useState<SearchResultItem | null>(
    null,
  );
    ...
    if (isMikro) {
      const formattedJak =
        jakRoutes.length > 0
          ? jakRoutes
              .map((j) =>
                j.startsWith("JAK-") ? j : j.replace(/JAK\s*/i, "JAK-"),
              )
              .join(", ")
          : "JAK-10";
  ...
  const filteredTransit = results.filter(
    (item) => item.type === "stop_or_route",
  );
  const filteredLandmarks = results.filter(
    (item) => item.type === "place_or_address",
  );

  return (
    <div className="relative flex h-full w-full flex-col overflow-hidden bg-[#F6F8FA] font-sans select-none">
      ...
      <section className="relative z-30 px-4 pt-6 pb-3">
      ...
      <section className="relative z-30 flex-1 [scrollbar-width:none] overflow-y-auto px-4 pb-4 [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden">
              ...
              <div className="mt-3 flex flex-col gap-2">
                {ambiguousItem.platforms?.map((plat) => (
                  <button
                    ...
                    type="button"
                    onClick={() =>
                      handlePlatformSelect(ambiguousItem, plat.label)
                    }
                  ...
                  {filteredTransit.map((stop, idx, arr) => {
                    ...
                    return (
                      <button
                        ...
                        className="relative flex w-full cursor-pointer items-center justify-between px-4 py-3 text-left transition-colors hover:bg-slate-50/80"
                      >
                        {/* Frame Icon Transportasi */}
                        <div
                          className={`mr-2.5 flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl ${meta.themeClass}`}
                        >
                          <svg
                            className="h-5 w-5"
                            fill="none"
                          ...
                          >
                            <path
                              strokeLinecap="round"
                              strokeLinejoin="round"
                              d="M8 17h.01M16 17h.01M4 11V6a2 2 0 012-2h12a2 2 0 012 2v5M4 11h16M4 11v6a2 2 0 002 2h1v1a1 1 0 001 1h1a1 1 0 001-1v-1h4v1a1 1 0 001 1h1a1 1 0 001-1v-1h1a2 2 0 002-2v-6M8 8h8"
                            />
                          </svg>
                        </div>
                        ...
                        <div className="flex-1 pr-1.5">
                          <h3 className="text-[12.5px] leading-tight font-semibold whitespace-nowrap text-slate-800">
                            {stop.title}
                          </h3>
                          <p className="mt-1 text-[10.5px] whitespace-nowrap text-slate-400">
                            {meta.subtitle}
                          </p>
                        </div>
                        ...
                        {idx < arr.length - 1 && (
                          <div className="absolute right-4 bottom-0 left-4 h-[1px] bg-[#E2E8F0]" />
                        )}
                      </button>
                    );
                  })}
                ...
                <div className="pt-3 pb-1">
                  ...
                  {filteredLandmarks.map((landmark, idx, arr) => (
                    <button
                      ...
                      className="relative flex w-full cursor-pointer items-center justify-between px-5 py-3 text-left transition-colors hover:bg-slate-50/80"
                    >
                      {/* Pin Icon + Teks */}
                      <div className="flex min-w-0 flex-1 items-center gap-3 pr-2">
                        ...
                        <div className="min-w-0 flex-1">
                          <h3 className="truncate text-[13px] leading-tight font-semibold text-slate-800">
                            {landmark.title}
                          </h3>
                      ...
                      {/* Garis Pembatas Baris */}
                      {idx < arr.length - 1 && (
                        <div className="absolute right-5 bottom-0 left-5 h-[1px] bg-[#E2E8F0]" />
                      )}
                    </button>
                  ))}
                </div>
...
function TruthBadge({ status }: { status: VerificationStatus }) {
  if (status === "Terverifikasi") {
    return (
      <span className="inline-flex items-center gap-1 rounded-md border border-emerald-500 bg-emerald-50/40 px-1.5 py-0.5 text-[10px] leading-none font-semibold text-emerald-600">
        <svg
          className="h-2.5 w-2.5 shrink-0 stroke-[2.5]"
          fill="none"
          stroke="currentColor"
          viewBox="0 0 24 24"
        >
  ...
  if (status === "Perlu dicek") {
    return (
      <span className="inline-flex items-center rounded-md border border-amber-500 bg-amber-50/50 px-1.5 py-0.5 text-[10px] leading-none font-semibold text-amber-700">
  ...
  return (
    <span className="inline-flex items-center rounded-md border border-slate-400 bg-slate-50/50 px-1.5 py-0.5 text-[10px] leading-none font-semibold text-slate-500">
