...
import type {
  GtfsIssueClassification,
  GtfsSnapshot,
  GtfsValidationIssue,
} from "~/core/ingestion/gtfsTypes";

type NormalizedSnapshot = GtfsSnapshot;
...
export async function writeValidationIssues(
  ...
  const data: Prisma.GtfsValidationIssueCreateManyInput[] = issues.map(
    (issue, issueIndex) => ({
      snapshotId,
      issueIndex,
      code: issue.code,
      classification: toPersistenceClassification(issue.classification),
...
function toPersistenceClassification(
  classification: GtfsIssueClassification,
): "BLOCKER" | "WARNING" {
  return classification === "blocker" ? "BLOCKER" : "WARNING";
}
